...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
	...
	return (
		<div className="w-full font-sans px-6 md:px-10" ref={containerRef}>
			...
			<div ref={ref} className="relative max-w-7xl mx-auto pb-28">
					...
					>
						<div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
							<div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-secondary flex items-center justify-center">
								<div className="h-4 w-4 rounded-full bg-foreground border border-secondary p-2" />
							</div>
				...
				>
					<motion.div
						...
						className="absolute inset-x-0 top-0  w-[2px] bg-linear-to-t from-secondary via-primary to-transparent from-[0%] via-[10%] rounded-full"
					/>
				</div>
			</div>
		</div>
